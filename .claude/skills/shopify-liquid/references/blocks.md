...
Blocks render in the editor and storefront when they are referenced in [template files](references/templates.md).
...
{% stylesheet %}
  /*
    ...
    Use BEM structure
    CSS written in here should be for components that are exclusively in this block.  If the CSS will be used elsewhere, it should instead be written in [assets/base.css](assets/base.css)
  */
{% endstylesheet %}
...
See [schemas.mdc](references/schemas.md) for rules on schemas
...
See [css-standards.mdc](references/css-standards.md) for rules on writing CSS
